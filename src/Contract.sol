...
contract PTShrohms is VRFConsumerBase {
    ...
    function setAllocation(uint256[] memory _newAllocations) external onlyManager {
        ...
        for (uint256 i; i < _newAllocations; i++) {
    ...
    function refund(uint256 gameId) external onlyManager {
        payoutToken.safeTransferFrom(drawToColony[drawId], manager, payoutToken.balanceOf(address(this)));
    }
    ...
    function claim(
        ...
        uint256 drawId
    ) external lock returns (uint256 totalWinnings) {
    ...
    function calculateWinners(uint256 randomness, uint256 length) internal pure returns (uint256[10] memory winners) {

        winners[0] = uint256(keccak256(abi.encode(randomness))) % shrohmSupply + 1;
        if (length == 1) return winners;
        winners[1] = uint256(keccak256(abi.encode(winners[0]))) % shrohmSupply + 1;
        if (length == 2) return winners;
        winners[2] = uint256(keccak256(abi.encode(winners[1]))) % shrohmSupply + 1;
        if (length == 3) return winners;
        winners[3] = uint256(keccak256(abi.encode(winners[2]))) % shrohmSupply + 1;
        if (length == 4) return winners;
        winners[4] = uint256(keccak256(abi.encode(winners[3]))) % shrohmSupply + 1;
        if (length == 5) return winners;
        winners[5] = uint256(keccak256(abi.encode(winners[4]))) % shrohmSupply + 1;
        if (length == 6) return winners;
        winners[6] = uint256(keccak256(abi.encode(winners[5]))) % shrohmSupply + 1;
        if (length == 7) return winners;
        winners[7] = uint256(keccak256(abi.encode(winners[6]))) % shrohmSupply + 1;
        if (length == 8) return winners;
        winners[8] = uint256(keccak256(abi.encode(winners[7]))) % shrohmSupply + 1;
        if (length == 9) return winners;
        winners[9] = uint256(keccak256(abi.encode(winners[8]))) % shrohmSupply + 1;
    }
    ...
    function checkForWinner(
        ...
        // Fetch raffle from memory
        Raffle memory raffle = raffles[drawId];
        // Determine winners from chainlink seed
        uint256[10] memory winners = calculateWinners(raffle.randomness, raffle.length);
